...
const HAND_CONTROLS_GESTURE = "hand-controls-gesture";

AFRAME.registerComponent(HAND_CONTROLS_GESTURE, {
  schema: {
    color: { default: "#333", type: "color" },
    hand: { default: "left" },
    handModelStyle: {
      default: "lowPoly",
   ...
   */
  handleGestureChanged: function (self) {
    const button = self.data.button;
    const evt = self.data.buttonEvent;

    this.isRemote = (this.el.getDOMAttribute("only-remote-controlled") == "");

    if (button != "none" && evt != "none") self.el.object3D.visible = true;
    ...
    if (evt.indexOf("touch") === 0) {
      // Update touch object.
      if (isTouched === self.touchedButtons[button]) {
        return;
      }
      self.touchedButtons[button] = isTouched;
    } else {
      // Update button object.
      if (isPressed === self.pressedButtons[button]) {
        return;
      }
      self.pressedButtons[button] = isPressed;
    }

    // Determine the gesture.
    lastGesture = self.gesture;
    self.gesture = self.determineGesture();

    self.animateGesture(self.gesture, lastGesture);

    // Emit events.
    self.emitGestureEvents(self.gesture, lastGesture);

    // Same gesture.
    if (self.gesture === lastGesture) {
      return;
    }
  },
  ...
  update: function(oldData) {
    ...
    if (
      (data.button != oldData.button ||
        data.buttonEvent != oldData.buttonEvent)
    ) {

      this.handleGestureChanged(this);
    }

    if(this.isNotInitialized)
    // Set model.
      if (hand !== oldData.hand) {
  ...
  handleButton: function(button, evt) {
    this.el.setAttribute(HAND_CONTROLS_GESTURE, {
      button: button,
      buttonEvent: evt
    });
  },
  ...
  determineGesture: function() {
    ...
    var isVive = isViveController(this.el.components["tracked-controls"], this);
...
function isViveController(trackedControls, self) {
  if (self.data.controller === "vive") return true;
  ...
  if(isVive) self.el.setAttribute(HAND_CONTROLS_GESTURE, {controller: "vive"})
  return isVive;
}
